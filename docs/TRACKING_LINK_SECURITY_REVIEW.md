...
Les phases publiques ont ensuite réussi : déploiement compatible, compteur protégé complet, passage à `encrypted_only`, zéro token en clair, smoke test Railway et navigateur mobile, démo en `404`, ancien lien remplacé invalide et rafale contrôlée produisant `429` avec `Retry-After`. La décision devient **GO pour le pilote à une seule réplique**. Le lancement à plusieurs répliques reste **NO-GO** tant que les quotas ne sont pas stockés dans Redis. La clé dédiée `TRACKING_TOKEN_SECRET` est désormais configurée et ne doit jamais être remplacée sans migration versionnée.
