...
// Libellés FRANÇAIS + type par colonne et par jeu de données. Source unique de
// vérité pour l'en-tête des exports (CSV/XLSX simple) et pour le générateur
// « premium ». Le type sert au formatage (date / nombre / texte).
const DATASET_COLUMN_DEFS = Object.freeze({
  operations: Object.freeze({
    order_id: { label: 'ID commande', type: 'texte' },
    created_at: { label: 'Date de création', type: 'date' },
    requested_window: { label: 'Créneau souhaité', type: 'texte' },
    status: { label: 'Statut', type: 'texte' },
    destination_zone: { label: 'Zone', type: 'texte' },
    driver_reference: { label: 'Livreur (réf.)', type: 'texte' },
    run_reference: { label: 'Tournée (réf.)', type: 'texte' },
    delivered_at: { label: 'Livrée le', type: 'date' },
    delivery_duration_minutes: { label: 'Durée (min)', type: 'nombre' },
    payment_status: { label: 'Statut de paiement', type: 'texte' },
    incident_count: { label: "Nb d'incidents", type: 'nombre' },
    customer_name: { label: 'Client', type: 'texte' },
    customer_phone: { label: 'Téléphone', type: 'texte' },
    delivery_address: { label: 'Adresse de livraison', type: 'texte' },
    delivery_instructions: { label: 'Instructions', type: 'texte' },
  }),
  customers: Object.freeze({
    customer_reference: { label: 'ID client', type: 'texte' },
    destination_zone: { label: 'Zone', type: 'texte' },
    order_count: { label: 'Nb de commandes', type: 'nombre' },
    completed_order_count: { label: 'Commandes livrées', type: 'nombre' },
    last_order_at: { label: 'Dernière commande', type: 'date' },
    customer_name: { label: 'Nom', type: 'texte' },
    customer_phone: { label: 'Téléphone', type: 'texte' },
  }),
  incidents: Object.freeze({
    incident_id: { label: 'ID incident', type: 'texte' },
    order_id: { label: 'Commande', type: 'texte' },
    category: { label: 'Type', type: 'texte' },
    severity: { label: 'Gravité', type: 'texte' },
    status: { label: 'Statut', type: 'texte' },
    opened_at: { label: 'Ouvert le', type: 'date' },
    resolved_at: { label: 'Résolu le', type: 'date' },
    resolution_code: { label: 'Résolution', type: 'texte' },
  }),
  routes: Object.freeze({
    run_reference: { label: 'ID tournée', type: 'texte' },
    service_date: { label: 'Date de service', type: 'date' },
    driver_reference: { label: 'Livreur (réf.)', type: 'texte' },
    status: { label: 'Statut', type: 'texte' },
    stop_count: { label: "Nb d'arrêts", type: 'nombre' },
    order_count: { label: 'Nb de commandes', type: 'nombre' },
    started_at: { label: 'Débutée le', type: 'date' },
    completed_at: { label: 'Terminée le', type: 'date' },
  }),
});

// Libellé FR d'une colonne (repli sur la clé technique si non répertoriée).
function columnLabel(dataset, key) {
  const def = DATASET_COLUMN_DEFS[dataset] && DATASET_COLUMN_DEFS[dataset][key];
